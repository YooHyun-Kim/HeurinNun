...
from image_classifier.resnet import (
    build_resnet,
    preprocess_image as resnet_preprocess,
    get_image_features as resnet_get_features
)
from image_classifier.densenet import (
    build_densenet,
    preprocess_image as densenet_preprocess,
    get_image_features as densenet_get_features
)
...
def initialize_model(model_type: str):
    ...
    if model_type == 'resnet':
        ...
        get_image_features = lambda x: resnet_get_features(x, model, device)
    elif model_type == 'densenet':
        model = build_densenet(feature_only=False).to(device).eval()
        preprocess_image = densenet_preprocess
        get_image_features = lambda x: densenet_get_features(x, model, device)
    elif model_type == 'tip_adapter':
        # Tip-Adapter는 내부 predict 함수만 사용
        model = None
        preprocess_image = None
        get_image_features = None
    else:
...
def classify_pdf_document(pdf_path: str, return_pages: bool=False):
    ...
    has_text = has_image = image_has_text = False
    full_text = ""
            ...
            for img_idx, img_meta in enumerate(images, start=1):
                ...
                try:
                    if model is not None:
                        ...
                        # Tip-Adapter 경로
                        labels = predict_image_tip_adapter(img)  # ex) ['흐름도'] or ['기타'] or []
                        # '기타' 일 경우 스킵, 그 외 정상 태그만 append
                        if labels and labels[0] != "기타":
                            page_img_preds.append(labels[0])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='PDF 분류기 (ResNet / DenseNet / Tip-Adapter)'
    )
